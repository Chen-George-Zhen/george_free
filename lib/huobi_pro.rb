...
class HuobiPro
  ...
  def initialize
    @access_key = ENV["HPRO_ACCESS_KEY"]
    @secret_key = ENV["HPRO_SECRECT_KEY"]
    @signature_version = "2"
    @account_id = ENV["HPRO_ACCOUNT_ID"]
  ...
  def custom_new_order(symbol,side,count)
    params ={
      "account-id" => @account_id,
      "amount" => count,
      "source" => "api",
...
# huobi.market_trade("eosusdt", "step0")["tick"]["bids"].first.first //eos 最新买价(可卖出价)
# huobi.market_trade("eosusdt", "step0")["tick"]["asks"].first.first //eos 最新卖价(可买入价)

# huobi.balances["data"]["list"].select{|x| x["currency"] == "eos" && x["type"] == "trade"}.first["balance"] //eos 交易余额
# huobi.balances["data"]["list"].select{|x| x["currency"] == "usdt" && x["type"] == "trade"}.first["balance"] //usdt 交易余额

# huobi.custom_new_order("eosusdt","buy",1) // 市价 用 usdt 买入 eos (1usd)
# huobi.custom_new_order("eosusdt","sell",1) // 市价 卖出 eos 获得 usdt (卖出 1eos)
